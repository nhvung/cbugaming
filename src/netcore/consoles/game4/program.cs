            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(folderPath))
                {
                    ...
                    if (folder.Exists)
                    {
                        ...
                        var files = folder.GetFiles("*_car.*");
                        if (files?.Length > 0)
                        {
                            ...
                            foreach (var file in files)
                            {
                                ...
                                string relativeCarPath = $"/{parentFolder.Name}/{file.FullName.Substring(parentFolder.FullName.Length + 1).Replace("\\", "/")}";
                                string relativePlatePath = $"/{parentFolder.Name}/{plateFile.FullName.Substring(parentFolder.FullName.Length + 1).Replace("\\", "/")}";

                                outputContent oItem = new outputContent(txtFileName, $"{relativeCarPath}{Environment.NewLine}{relativePlatePath}", file, plateFile);
