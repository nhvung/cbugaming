...
namespace VSSystem.IO.Extensions
{
    public static class DirectoryInfoExtension
    {
        ...
        public static void BackupFolder(this DirectoryInfo sourceFolder, bool overwrite, string defaultName = "", ExcludeCondition excludeCondition = null, bool compresss = false, Action<string> executeFileLog = null)
        {

            string bkFolderName = defaultName;
            if (string.IsNullOrWhiteSpace(defaultName))
            {
                bkFolderName = DateTime.Now.ToString("yyyyMMdd.HHmm");
            }
            bkFolderName = "/@bk/" + bkFolderName;
            DirectoryInfo destinationFolder = new DirectoryInfo(sourceFolder.FullName + bkFolderName);
            _CopyTo(sourceFolder, destinationFolder, overwrite, excludeCondition, executeFileLog);
            if (compresss)
            {
                try
                {
                    string zipFilePath = $"{sourceFolder.FullName + bkFolderName}.zip";
                    ZipFile.Compress(destinationFolder.FullName, zipFilePath, System.Text.Encoding.UTF8);
                    destinationFolder = new DirectoryInfo(destinationFolder.FullName);
                    destinationFolder.Empty(true);
                }
                catch { }
            }
        }
